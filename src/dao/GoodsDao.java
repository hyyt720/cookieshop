...
//        return r.query(sql, new MapHandler());
    ...
    public int getStock(int id) throws SQLException {
        ...
        Integer stock = r.query(sql, new ScalarHandler<Integer>(), id);
        return stock;
    }

    public void setStock(int id, int stock) throws SQLException {
        QueryRunner r = new QueryRunner(DataSourceUtils.getDataSource());
        String sql = "update goods set stock = ? where id = ?";
        r.update(sql,stock,id);
    }

    public String getName(int id) throws SQLException {
        QueryRunner r = new QueryRunner(DataSourceUtils.getDataSource());
        String sql = "select name from goods where id = ?";
        return (String) r.query(sql, new ScalarHandler<>(), id);
    }

    public void addBrowsing(Browsing b) throws SQLException {
        QueryRunner r = new QueryRunner(DataSourceUtils.getDataSource());
        String sql = "insert into browsing(username,datetime,goods_name,type) values(?,?,?,?)";
        r.update(sql,b.getUsername(),b.getDatetime(),b.getGoods_name(),b.getType());
    }
}
